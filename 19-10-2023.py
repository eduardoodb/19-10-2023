...
class Professor:
    def __init(self, nome, turno):
        self.nome = nome
        self.turno = turno

    def salvar(self):
        query = "INSERT INTO Professores (Nome,TURNO) VALUES (%s, %s)"
        values = (self.nome,self.turno)
        cursor.execute(query, values)
        db.commit()
...
class Turma:
    def __init__(self, nome_turma, professor_id,sala):
        self.nome_turma = nome_turma
        self.professor_id = professor_id
        self.sala = sala

    def salvar(self):
        query = "INSERT INTO Turmas (NomeTurma, ProfessorID,SALA) VALUES (%s, %s, %s)"
        values = (self.nome_turma, self.professor_id,self.sala)
        cursor.execute(query, values)
        db.commit()
...
class Disciplina:
    def __init__(self, nome_disciplina, turma_id,horarios):
        self.nome_disciplina = nome_disciplina
        self.turma_id = turma_id
        self.horarios = horarios

    def salvar(self):
        query = "INSERT INTO Disciplinas (NomeDisciplina, TurmaID, HORARIOS) VALUES (%s, %s,%s)"
        values = (self.nome_disciplina, self.turma_id, self.horarios)
        cursor.execute(query, values)
        db.commit()
...
class UnidadeEscolar:
    def __init__(self, nome_unidade, endereco,cep):
        self.nome_unidade = nome_unidade
        self.endereco = endereco
        self.cep = cep

    def salvar(self):
        query = "INSERT INTO UnidadesEscolares (NomeUnidade, Endereco) VALUES (%s, %s,%s)"
        values = (self.nome_unidade, self.endereco, self.cep)
        cursor.execute(query, values)
        db.commit()
